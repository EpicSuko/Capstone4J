...
public class CapstoneX86Details extends CapstoneArchDetails<CapstoneX86Details.X86Operand> {
    ...
    /**
     * Returns operands that have the specified access type.
     * <p>
     * This method filters the instruction's operands based on their memory access type
     * (read, write, or read-write). It examines the access flags of each operand and
     * returns those that match the specified access type.
     * <p>
     * This is particularly useful for identifying which operands are reading from or
     * writing to memory or registers, which is essential for data flow analysis and
     * register dependency tracking.
     *
     * @param accessType The type of access to filter by (READ, WRITE, or READ_WRITE)
     * @return An array of operands that have the specified access type
     * @see CapstoneAccessType
     * @see X86Operand#getAccess()
     */
    @Override
    X86Operand[] getOperandsOfAccess(CapstoneAccessType accessType) {
        X86Operand[] operands = getOperands();
        List<X86Operand> result = new ArrayList<>();
        for(X86Operand operand : operands) {
            if((operand.getAccess() & accessType.getValue()) != 0) {
                result.add(operand);
            }
        }
        return result.toArray(new X86Operand[0]);
    }

    /**
     * Extracts register IDs from an X86 operand.
     * <p>
     * This method identifies all registers used in a specific operand and returns
     * their IDs as an array. For X86 architecture, different operand types contain
     * different register references:
     * <ul>
     *   <li>For register operands (REG): The register ID itself</li>
     *   <li>For memory operands (MEM): Base, index, and segment registers</li>
     *   <li>For immediate operands (IMM): No registers are involved</li>
     * </ul>
     * <p>
     * This extraction is particularly important for comprehensive register usage analysis,
     * as it allows tracking all registers that are implicitly or explicitly accessed by
     * an instruction.
     *
     * @param operand The operand to extract registers from (must be an X86Operand)
     * @return An array of register IDs found in this operand, or an empty array if none
     * @throws IllegalArgumentException If the operand is not an instance of X86Operand
     * @see X86OperandType
     * @see X86Reg
     */
    @Override
    int[] extractRegistersFromOperand(Object operand) {
        if(!(operand instanceof X86Operand)) {
            throw new IllegalArgumentException("Operand is not an instance of X86Operand");
        }

        X86Operand x86Operand = (X86Operand) operand;

        List<Integer> registers = new ArrayList<>();

        switch (x86Operand.getType()) {
            case REG:
                registers.add(x86Operand.getReg().getValue());
                break;
            case MEM:
                X86OpMem mem = x86Operand.getMem();
                if(mem.getBase() != null) {
                    registers.add(mem.getBase().getValue());
                }
        ...
        int[] regs = new int[registers.size()];
        for (int i = 0; i < registers.size(); i++) {
            regs[i] = registers.get(i);
        }
